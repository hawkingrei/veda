...
type Jerusalem struct {
	...
	Metrics struct {
		...
		Counters struct {
			JerusalemIndexThreadPoolRunning struct {
				Count int `json:"count"`
			} `json:"jerusalem.indexThreadPool.running"`
		} `json:"counters"`
		Histograms struct {
		} `json:"histograms"`
		Meters struct {
			JerusalemIndexThreadPoolCompleted struct {
				Count    int     `json:"count"`
			...
			} `json:"jerusalem.indexThreadPool.completed"`
			JerusalemIndexThreadPoolSubmitted struct {
				Count    int     `json:"count"`
				M15Rate  float64 `json:"m15_rate"`
				M1Rate   float64 `json:"m1_rate"`
				M5Rate   float64 `json:"m5_rate"`
				MeanRate float64 `json:"mean_rate"`
				Units    string  `json:"units"`
			} `json:"jerusalem.indexThreadPool.submitted"`
		} `json:"meters"`
		Timers struct {
			JerusalemIndexThreadPoolDuration struct {
				Count         int     `json:"count"`
			...
			} `json:"jerusalem.indexThreadPool.duration"`
		} `json:"timers"`
	} `json:"metrics"`
}

func (s *Jerusalem) toCollectData(dataChan *chan *CollectData) {
